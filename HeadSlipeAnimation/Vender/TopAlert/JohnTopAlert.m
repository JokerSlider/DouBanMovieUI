...
@property (nonatomic,weak) UILabel *pointLB;//小标题
...
- (void)createAlert{
    ...
    UIImageView *alertIMGV = [[UIImageView alloc]initWithFrame:CGRectMake(10, 10, 10, 10)];
    ...
    */
    //设置提示信息
    UILabel *alertMsg = [[UILabel alloc]initWithFrame:CGRectMake(CGRectGetMaxX(alertIMGV.frame) + 10, 5, self.frame.size.width - CGRectGetMaxX(alertIMGV.frame) - 30, 42)];
    alertMsg.textColor = [UIColor whiteColor];
    alertMsg.textAlignment = NSTextAlignmentLeft;
    alertMsg.font = [UIFont systemFontOfSize:14.0f];
    alertMsg.numberOfLines = 2;
...
- (void)showAlertMessage:(NSString *)msg andTitle:(NSString *)title alertType:(MessageType)type{
    ...
    if ([title isEqualToString:@""]) {
        CGRect frame = CGRectMake(10, 10 +(self.frame.size.height - 40)/ 2 , 20, 20);

...
@property (nonatomic, strong) YunUploadBottomView *yunUploadBottomView;
@end
...
- (void)viewWillAppear:(BOOL)animated{
    ...
    [_yunUploadBottomView setLocationBtnTitle:[[XGWebDavUploadManager shareUploadManager].selctPath lastPathComponent]];
}
...
- (void)setup
{
    ...
    _yunUploadBottomView = [[YunUploadBottomView alloc] initWithFrame:CGRectMake(0, heght-116, width, 116)];
    WEAKSELF;
    _yunUploadBottomView.uploadClick = ^(){
        XGWebDavUploadManager *manager = [XGWebDavUploadManager shareUploadManager];
        //拿到所有选择的照片，一一处理。
        for (HXPhotoModel *model in weakSelf.manager.selectedPhotos) {
//            HXPhotoModel *model = weakSelf.manager.selectedPhotos[0];
            PHImageRequestOptions *options = [[PHImageRequestOptions alloc] init];
            // 同步获得图片,
            options.synchronous = YES;
            [[PHImageManager defaultManager] requestImageForAsset:model.asset targetSize:model.imageSize contentMode:PHImageContentModeDefault options:options resultHandler:^(UIImage * _Nullable result, NSDictionary * _Nullable info) {
                NSLog(@"%@", result);
                [manager addUploadUrl:info[@"PHImageFileURLKey"]];
            }];
        }
      
    };
    
    _yunUploadBottomView.locationClick = ^{
    ...
    [self.view addSubview:_yunUploadBottomView];

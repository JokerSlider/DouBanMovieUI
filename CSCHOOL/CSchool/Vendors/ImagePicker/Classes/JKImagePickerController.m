...
- (void)viewDidLoad {
    ...
    if(author == ALAuthorizationStatusRestricted || author ==ALAuthorizationStatusDenied){
        ...
        [self showErrorView:@"此应用程序没有权限来访问您的照片\n您可以在“隐私设置”中启用访问" andImageName:nil];
        return;
    }
...
- (void)cancelEventDidTouched
{
    ...
    }else if (_imagePickerCancelBlock){
        _imagePickerCancelBlock(self);
    }
}
...
- (void)finishPhotoDidSelected
{
    ...
    }else if (_imagePickerDidSelectAssetsBlock){
        _imagePickerDidSelectAssetsBlock(self, self.selectedAssetArray, _selectButton.selected);
    }
}
...
- (void)startPhotoAssetsViewCell:(JKAssetsViewCell *)assetsCell
{
    ...
    if(authStatus == AVAuthorizationStatusRestricted || authStatus ==AVAuthorizationStatusDenied){
        ...
        //无权限
        [[[UIAlertView alloc] initWithTitle:@"获取权限失败，请在设置->隐私->相机中打开"
                                    message:nil
                                   delegate:nil
                          cancelButtonTitle:@"好的"
                          otherButtonTitles:nil, nil] show];

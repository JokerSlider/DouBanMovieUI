...
//    int c = [self.player.media storeCookie:@"cookie_testtest" forHost:@"192.168.80.243" path:@"/remote.php/webdav/"];
    
    NSHTTPCookieStorage *cookieJar = [NSHTTPCookieStorage sharedHTTPCookieStorage];
    for (NSHTTPCookie *cookie in [cookieJar cookies]) {
        NSLog(@"%@", cookie);
        
        int c = [self.player.media storeCookie:[NSString stringWithFormat:@"%@=%@",cookie.name, cookie.value] forHost:cookie.domain path:cookie.path];
        NSLog(@"--------------%d",c);

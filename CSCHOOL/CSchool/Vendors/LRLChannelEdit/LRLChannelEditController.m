...
//每行频道的个数
#define ButtonCountOneRow 3
#define ButtonHeight (ButtonWidth * 3/9)
...
@property (nonatomic, strong) NSArray *topDicArray;

@end

@implementation LRLChannelEditController

-(id)initWithTopDataSource:(NSArray *)topDataArr andBottomDataSource:(NSArray<ChannelUnitModel *> *)bottomDataSource andInitialIndex:(NSInteger)initialIndex{
    if (self = [super init]) {
        
        self.topDicArray = topDataArr;
...
//        _dataArray = responseObject[@"data"];
        ...
        NSPredicate * filterPredicate = [NSPredicate predicateWithFormat:@"NOT (SELF IN %@)",self.topDicArray];
        
        NSArray * filter = [dataArray filteredArrayUsingPredicate:filterPredicate];
        
        self.bottomDataSource = [NSMutableArray array];
        for (NSDictionary *dic in filter) {
        ...
        for (NSDictionary *dic in self.topDicArray) {
            ChannelUnitModel *model = [[ChannelUnitModel alloc] init];
            [model yy_modelSetWithDictionary:dic];
...
-(void)configUI{
    ...
    for (int i = 0; i < self.topDataSource.count; ++i) {
        ...
        if (i < 1) { //位于前两个的频道不添加任何手势, 并且文字颜色为灰色
            touchView.contentLabel.textColor = Base_Orange;

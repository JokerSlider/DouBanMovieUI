...
- (void)AddSegumentArray:(NSArray *)SegumentArray{
    ...
    for (int i = 0; i < SegumentArray.count; i++) {
        ...
        if (i == 0) {
            buttonDownView =[[UIView alloc]initWithFrame:CGRectMake(i * witdthFloat+29, self.bounds.size.height - 2, witdthFloat*0.5, 2)];
...
-(void)selectTheSegument:(NSInteger)segument{
    ...
    if (selectSeugment != segument) {
        ...
        [UIView animateWithDuration:0.2 animations:^{
            ...
            [buttonDownView setFrame:CGRectMake(segument * witdthFloat+29,self.bounds.size.height - 2, witdthFloat*0.5, 2)];
        }];

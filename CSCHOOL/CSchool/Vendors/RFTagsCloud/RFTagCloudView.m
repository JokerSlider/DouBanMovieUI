...
- (void)addPages:(NSArray *)array{
    self.totalPages++;
    
    int i = self.totalPages-1;
    
    RFTagCloudPageView *pageView = [[RFTagCloudPageView alloc] initWithFrame:CGRectMake(0, 0, self.frame.size.width, self.frame.size.height)];
    pageView.delegate = self;
    pageView.tag = kRFTagCloudPageViewStartTag + i;
    pageView.alpha = 0;
    [pageView drawCloudWithWords:array colors:self.colors rowHeight:self.rowHeight isHandle:YES];
    [self addSubview:pageView];
//    [UIView animateWithDuration:kRFTagCloudPageAnimationDur animations:^{
//        [self handleTheValViewWhenToRightWithDesPer:0.2];
//    }];
//    [self handleTheValViewWhenToLeftWithDesPer:self.per andChangeCurPageFrameOrNot:YES];
    [self autoHandleViewToChange];
}
...
- (void)handleTheValViewWhenToLeftWithDesPer:(float )desPer andChangeCurPageFrameOrNot:(BOOL) isChange{
    ...
    NSLog(@"%f,%d",desPer, isChange);
}

- (void)autoHandleViewToChange{
    [self handleTheValViewWhenBegan];
    self.preDisplayView.frame = CGRectZero;
    self.preDisplayView.center = self.center;
...
//        self.curPageView.frame = CGRectMake(0, 0, self.frame.size.width, self.frame.size.height);
        self.curPageIndex = [self validPageValue:self.curPageIndex+1];
//        [UIView animateWithDuration:kRFTagCloudPageAnimationDur animations:^{
//            [self handleTheValViewWhenToLeftWithDesPer:0 andChangeCurPageFrameOrNot:YES];
//        }];
    }];
}

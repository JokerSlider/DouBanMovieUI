...
- (void)getData {
    ...
    NSString *path = [[NSBundle mainBundle] pathForResource:@"province_data3" ofType:@"xml"];
    NSString *dataStr;
    if (!_dataArr) {
...
{
    if (![address isEqualToString:@""]&&[address rangeOfString:@"-"].location !=NSNotFound) {
        NSArray *array = [address componentsSeparatedByString:@"-"];
        NSString *_province = array[0];
        NSString *_city = array[1];
        NSString *_area = array[2];
        NSMutableArray *_provinceNameArray = [NSMutableArray array];
        NSMutableArray *_cityNameArray  = [NSMutableArray array];
        NSMutableArray *_townNameArray = [NSMutableArray array];
        for (NSObject *obj in self.dataArr) {
            AddressModel *addressModel = (AddressModel *)obj;
            [_provinceNameArray addObject:addressModel.zipcode];
        }
        NSInteger provinceIndex= [_provinceNameArray indexOfObject:_province]; //获取在第几个
        _selectedIndex_province = provinceIndex;
        [self reloadComponent:0];
        [self selectRow:provinceIndex inComponent:0 animated:YES];
        
        AddressModel *addressModel = self.dataArr[provinceIndex];
        for (NSObject *obj in addressModel.list) {
            CityModel *cityModel =(CityModel *)obj;
            [_cityNameArray addObject:cityModel.zipcode];
        }
        NSInteger cityIndex= [_cityNameArray indexOfObject:_city]; //获取在第几个
        self.selectedIndex_city = cityIndex;
        [self reloadComponent:1];
        [self selectRow:cityIndex inComponent:1 animated:YES];
        
        CityModel *cityModel = addressModel.list[self.selectedIndex_city];
        for (NSObject *obj in cityModel.list) {
            DistrictModel *districtModel = (DistrictModel *)obj;
            [_townNameArray addObject:districtModel.zipcode];
        }
        NSInteger areaIndex= [_townNameArray indexOfObject:_area]; //获取在第几个
        self.selectedIndex_area = areaIndex;
        [self reloadComponent:2];
        [self selectRow:areaIndex inComponent:2 animated:YES];
    
    }
}
//-(void)setJobAddress:(NSString *)jobAddress
//{
//    if (jobAddress) {
//        NSArray *array = [jobAddress componentsSeparatedByString:@"-"];
//        NSString *_province = array[0];
//        NSString *_city = array[1];
//        NSString *_area = array[2];
//        NSMutableArray *_provinceNameArray = [NSMutableArray array];

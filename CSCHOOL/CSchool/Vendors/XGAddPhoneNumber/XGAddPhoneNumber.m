...
@implementation XGAddPhoneNumber

// 单列模式
+ (XGAddPhoneNumber*)shareControl
{
    static XGAddPhoneNumber *instance;
    @synchronized(self) {
        if(!instance) {
            instance = [[XGAddPhoneNumber alloc] init];
        }
    }
    return instance;
}
...
{
    return [[XGAddPhoneNumber shareControl] addContactName:name phoneNum:num withLabel:label];
}
...
{
    return [[XGAddPhoneNumber shareControl] existPhone:phoneNum];
}

...
- (void)loadRoomMsgsWith:(NSString *)roomJidStr{
    ...
    NSPredicate *pre = [NSPredicate predicateWithFormat:@"streamBareJidStr = %@ AND bareJidStr = %@ ",[HQXMPPUserInfo  shareXMPPUserInfo].jid, [NSString jidStrWithRoomName:roomJidStr]];
...
-(void)loadMsgsWith:(NSString *)userJid{
    ...
    self.messageArr = [NSMutableArray arrayWithArray:_resultsContr.fetchedObjects];
//    [HQXMPPManager shareXMPPManager].msgStor
}

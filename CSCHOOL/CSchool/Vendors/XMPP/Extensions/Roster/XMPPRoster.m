...
- (id)initWithRosterStorage:(id <XMPPRosterStorage>)storage dispatchQueue:(dispatch_queue_t)queue
{
	...
	NSParameterAssert(storage != nil);///闪退
    }else{
        
        NSLog(@"storage error"); //增加一层判断，防止切换用户偶尔出现闪退，但是会出现无好友列表情况。
    }
...
#pragma mark 新增方法  设置昵称和请求消息的方法
-(void)addUser:(XMPPJID *)jid withNickname:(NSString *)optionalName groups:(NSArray *)groups RequestMessageInfo:(NSString *)messaage mySchoolCode:(NSString *)myCode andFriendCode:(NSString *)friendCode  subscribeToPresence:(BOOL)subscribe
{
    if (jid == nil) return;
    ...
    NSXMLElement *item = [NSXMLElement elementWithName:@"item"];
    XMPPJID *schoolJid = jid;
    [item addAttributeWithName:@"jid" stringValue:[schoolJid bare]];

    [item addAttributeWithName:@"uid" stringValue:jid.user];
    if(optionalName)
    {
        [item addAttributeWithName:@"name" stringValue:optionalName];
    }
    if (myCode) {
        [item addAttributeWithName:@"uscode" stringValue:myCode];
    }
    if (friendCode) {
        [item addAttributeWithName:@"jscode" stringValue:friendCode];
    ...
    if(subscribe)
    {
        [self subscribePresenceToUser:jid withInviteInfo:messaage];
    }
...
- (void)subscribePresenceToUser:(XMPPJID *)jid withInviteInfo:(NSString *)messageInfo
{
    if (jid == nil) return;
    ...
    XMPPPresence *presence = [XMPPPresence presenceWithType:@"subscribe" to:[jid bareJID]];
    NSXMLElement *fatherEle = [NSXMLElement elementWithName:@"Message"];
    NSXMLElement *message = [NSXMLElement elementWithName:@"msg" stringValue:messageInfo];
    [fatherEle addChild:message];
    [presence addChild:fatherEle];
    [xmppStream sendElement:presence];

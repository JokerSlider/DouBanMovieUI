...
-(NSXMLElement *)configNewRoom:(XMPPRoom *)room{
    ...
    [p addChild:[NSXMLElement elementWithName:@"value" stringValue:@"0"]]; //history   不获取历史消息
    [x addChild:p];
    
    p=[NSXMLElement elementWithName:@"history"];
    [p addAttributeWithName:@"maxchars" stringValue:@"0"]; //不获取
//     [p addAttributeWithName:@"seconds" stringValue:intervalTime];//多少秒之前的数据
    [x addChild:p];
...
- (void)queryRooms{
    [muc discoverRoomsForServiceNamed:[NSString stringWithFormat:@"conference.%@",kDOMAIN]];
}
...
- (void)joinInChatRoom:(NSString *)roomJid withPassword:(NSString *)password{//
    ...
    NSTimeInterval timeCrash = [[[NSUserDefaults standardUserDefaults] objectForKey:@"terminateTime"] doubleValue];
    if (timeCrash) {
        NSTimeInterval timeStart = [[NSDate date] timeIntervalSince1970];
        int time = timeStart - timeCrash;
        
        NSXMLElement *p=[NSXMLElement elementWithName:@"history"];
        [p addAttributeWithName:@"seconds" stringValue:[NSString stringWithFormat:@"%d",time]];//多少秒之前的数据
        [element_x addChild:p];
    }
...
- (void)sendImageData:(NSString *)message inChatRoom:(NSString *)chatRoomJid{
    ...
    NSXMLElement *imageEl = [NSXMLElement elementWithName:@"image" stringValue:message];
...
- (void)xmppMUC:(XMPPMUC *)sender didDiscoverRooms:(NSArray *)rooms forServiceNamed:(NSString *)serviceName{
    ...
    for (XMPPElement * element in rooms) {
        [self.roomList addObject:element];
    }
    if (self.updateData) {
        self.updateData(nil);
    }

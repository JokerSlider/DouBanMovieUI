...
-(instancetype)initWithFrame:(CGRect)frame withDataArray:(NSArray *)dataArray withFont:(CGFloat)font {
    ...
    if (self) {
        ...
        //默认
        self.textNomalColor    = [UIColor grayColor];
        self.textSelectedColor = Base_Orange;
        self.lineColor = Base_Orange;
...
-(void)addSubSegmentView
{
    ...
    for (int i = 0 ; i < _dataArray.count ; i++) {
        ...
        if (i != _dataArray.count || i != 0) {
            UILabel *sepLine = [[UILabel alloc]initWithFrame:CGRectMake(kScreenWidth/2, 5, 0.8, self.frame.size.height-12)];
            sepLine.backgroundColor = [UIColor lightGrayColor];
            [self addSubview:sepLine];
            if (_dataArray.count>2||_dataArray.count==1) {
                sepLine.hidden = YES;
            }
...
//    if(_dataArray.count==1)  self.lineImageView.hidden = YES;
...
-(void)tapAction:(id)sender{
    ...
    [UIView animateWithDuration:0.2 animations:^{
        self.lineImageView.frame = CGRectMake(button.frame.origin.x+width/4, self.frame.size.height-2, width/2, 2);
    }];
...
-(void)selectIndex:(NSInteger)index
{
    ...
    for (UIButton *subButton in self.buttonsArray) {
        if (index != subButton.tag) {
            ...
            subButton.selected = YES;
            [UIView animateWithDuration:0.2 animations:^{
                self.lineImageView.frame = CGRectMake(subButton.frame.origin.x+width/4, self.frame.size.height-2, width/2, 2);
            }];
        }
    }
}

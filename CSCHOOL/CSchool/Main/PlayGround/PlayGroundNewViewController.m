//
//  PlayGroundNewViewController.m
//  CSchool
//
//  Created by 左俊鑫 on 16/4/13.
...
@interface PlayGroundNewViewController ()<ZWAdViewDelagate, XGAlertViewDelegate, UITableViewDelegate, UITableViewDataSource,UIScrollViewDelegate,UIWebViewDelegate,SelectVcDelegate>
{
...
@property (nonatomic,strong)ChatMessageViewController *messageVC;
@property (nonatomic, assign) BOOL isShowed; //是否已经显示此页面

@property (nonatomic, strong) AMapLocationManager *locationManager;
@end

@implementation PlayGroundNewViewController
...
#
    //有待检验
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(DestroySelfView) name:NotificationLogout object:nil];
    //加载Agent
    if (ISNEWVERSION) {
...
-(void)createBanner{
    //大视图
    _bannerView = ({
        ZWAdView *view = [[ZWAdView alloc]initWithFrame:CGRectMake(0, 0,kScreenWidth , LayoutHeightCGFloat(240))];
...
//    }];
    ...
    //新生是否弹出
    if ([AppUserIndex GetInstance].isBundPhone) {

        [self showBundPhone:nil];
        
        [AppUserIndex GetInstance].isBundPhone = NO;
    }
    
    //次月验证手机号，更改带宽出口
    if ([ConfigObject shareConfig].isShowChangePhone) {
        BasePhoneViewController *phoneVc = [[BasePhoneViewController alloc] init];
        phoneVc.phoneType = XGChangePhone;
    ...
    if (!_isShowed) {
        if ([AppUserIndex GetInstance].isNewEntry) {
            ...
            XGAlertView *alter = [[XGAlertView alloc] initWithTarget:self withTitle:@"提示" withContent:@"亲爱的2017级新同学,欢迎您加入山东建筑大学,进入菁彩校园,在这里您将享受到学校全方位的信息服务，包括学习、生活、娱乐、交友，菁彩伴随您的每一天，此处服务持续更新中。" WithCancelButtonTitle:@"确定" withOtherButton:nil];
            [alter show];
        }
        _isShowed = YES;
    }
    ...
    if ([AppUserIndex GetInstance].pushUserInfo) {
        [self pustAction:[AppUserIndex GetInstance].pushUserInfo];
        [AppUserIndex GetInstance].pushUserInfo =nil;
    }

}
#pragma  mark  销毁当前视图 重新登录的时候 重新创建
-(void)DestroySelfView
{
    [[NSNotificationCenter defaultCenter] removeObserver:self name:NotificationLogout object:nil];
    self.view = nil;
}
...
//    NSInteger numOfLine = 4;
    ...
    _btnTitleArray = [IndexManager shareConfig].titlesArray;
    ...
    [_mainScrollView setupAutoContentSizeWithBottomView:_newsTableView bottomMargin:0];
    ...
    .leftSpaceToView(_mainScrollView,0)
    .rightSpaceToView(_mainScrollView,0)
    .topSpaceToView(_userInfoView,1);
    ...
    [payBtn addTarget:self action:@selector(openSignVC) forControlEvents:UIControlEventTouchUpInside];
//    [payBtn setTitle:@"签到" forState:UIControlStateNormal];
    [payBtn setImage:[UIImage imageNamed:@"index_signBtn"] forState:UIControlStateNormal];
...
- (void)buttonShow{
    ...
    for (NSDictionary *dic in [AppUserIndex GetInstance].appListArray) {
        ...
        if ([dic[@"AI_VERSION"] integerValue] == 2) {
            //这里判断新生登录进行区分
            UIButton *btn = [self getNewButton];
//            [btn sd_setBackgroundImageWithURL:[NSURL URLWithString:dic[@"ICON"]] forState:UIControlStateNormal placeholderImage:[UIImage imageNamed:@"newLogin_jing"]];
            [btn sd_setImageWithURL:[NSURL URLWithString:dic[@"ICON"]] forState:UIControlStateNormal placeholderImage:[UIImage imageNamed:@"newLogin_jing"]];
            [btn setTitle:dic[@"AI_NAME"] forState:UIControlStateNormal];
            if ([dic[@"RSA_ENABLE"] boolValue]) {
                [btn addTarget:self action:@selector(newEntryAction:) forControlEvents:UIControlEventTouchUpInside];
            }else{
                [btn addTarget:self action:@selector(notOpenShow:) forControlEvents:UIControlEventTouchUpInside];
            }
            btn.tag = [dic[@"AG_ID"] integerValue];
            [_buttonShowArray addObject:btn];
            
        }else if (index < _buttonMutArray.count) {
            if ([dic[@"RSA_ENABLE"] boolValue]) {
...
//    if ([[AppUserIndex GetInstance].accountId isEqualToString:@"wzw"] || [[AppUserIndex GetInstance].accountId isEqualToString:@"yfcs01"]) {
//        _buttonShowArray = [NSMutableArray array];
//        _buttonShowArray = _buttonMutArray;
//    }
...
- (void)newEntryAction:(UIButton *)sender{
...
- (void)notOpenShow:(UIButton *)sender{
    if (sender.tag == 44) {
        XGAlertView *alert = [[XGAlertView alloc] initWithTarget:self withTitle:@"提示" withContent:@"手机验证将于近日开启，请您先激活通知书中提供的三张免费手机体验卡，做好准备，验证后将会为您提供及时权威的报到信息，敬请期待。" WithCancelButtonTitle:@"确定" withOtherButton:nil];
        [alert show];
    }else{
        XGAlertView *alert = [[XGAlertView alloc] initWithTarget:self withTitle:@"提示" withContent:@"添加中，将在不久开放~" WithCancelButtonTitle:@"确定" withOtherButton:nil];
        [alert show];
    }
    ...
    [NetworkCore requestNormalGET:TEST_URL parameters:nil success:^(NSURLSessionDataTask * _Nullable task, id  _Nullable responseObject) {
        ...
        [NetworkCore requestNormalGET:Epurl parameters:nil success:^(NSURLSessionDataTask * _Nullable task, id  _Nullable responseObject) {
            ...
            [NetworkCore requestNormalGET:@"http://1.1.1.1/?rand=0.619542766180218" parameters:nil success:^(NSURLSessionDataTask * _Nullable task, id  _Nullable responseObject) {
                ...
                //用户名
                AppUserIndex *userClass = [AppUserIndex GetInstance];
                
                NSString *username = userClass.accountId;
                //密码
                NSString *pwd = userClass.passWord;
    ...
    [NetworkCore requestOriginlTarget:url parameters:dic isget:NO block:^(id responseObject) {
        ...
        if ([result isEqualToString:@"success"]) {
            ...
            if (![keepaliveInterval isEqualToString:@"0"]) {
                //30分钟保活时间
                _netKeepTimer =  [NSTimer scheduledTimerWithTimeInterval:10*60 target:self selector:@selector(keepAlive) userInfo:nil repeats:YES];
                [_netKeepTimer  fire];
            }
        }else{
...
-(void)loadBannerImage{
    ...
    NSDictionary *dic = @{
                          @"rid":@"getBannerInfo",
                          @"newEntry":@([AppUserIndex GetInstance].isNewEntry)
                          };
...
{
    //定位结果
    NSLog(@"location:{loc:%f,%f; accuracy:%f}", location.coordinate.latitude, location.coordinate.longitude, location.horizontalAccuracy);
}
...
//    UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"" message:[NSString stringWithFormat:@"%ld", (long)btn.tag] delegate:self cancelButtonTitle:@"yes" otherButtonTitles: nil];
//    [alert show];
    [self buttonViewAction:btn];
}
...
#pragma mark topAlertDelegate
-(void)tapAlertViewBySingle
{
    ChatMianViewController *vc1 = [[ChatMianViewController alloc]init];

...
@interface OfficalInfoViewController ()<UIWebViewDelegate,UITableViewDataSource,UITableViewDelegate,XGAlertViewDelegate>
{
    NSString *_REDIRECT_URL;
    NSMutableArray *fileNameArr;//附件数组
    NSMutableArray *fileArr;//附件名
    UIView *topView;
...
@implementation OfficalInfoViewController
...
-(void)loadData
{
    ...
    WEAKSELF;
    [NetworkCore requestPOST:[AppUserIndex GetInstance].API_URL   parameters:@{@"rid":@"showInfomationOrDocumentInfoById",@"uid":_newsID,@"type":_type} success:^(NSURLSessionDataTask * _Nullable task, id  _Nullable responseObject) {
        [ProgressHUD dismiss];
        ...
        self.baseUrl =[dic objectForKey:@"content"];
        dispatch_queue_t queue = dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0);
        dispatch_async(queue, ^{
            [weakSelf  loadFIle];
        });
        _titleLabel.text = dic[@"title"];
        _timeLabel.text  = dic[@"releaseTime"];
        _deparMentLbel.text =[NSString stringWithFormat:@"%@",dic[@"releaseDepart"]];

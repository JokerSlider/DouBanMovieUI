...
-(void)drawRect:(CGRect)rect {
    ...
    for (NSInteger i = 0 ; i < self.emotionPages.count; i++) {
        ...
        for (NSInteger j = 0 ; j < items.count; j++) {
            ...
            NSString *imageName = [itemDic allValues][0];
...
//            [itemDic setObject:[dic allValues][0] forKey:[dic allKeys][0] ];
//        }
//        NSLog(@"%@",itemDic);
////        itemDic json
...
//            
//            [itemDic writeToFile:fileName atomically:YES];
//            NSLog(@"文件写入完成");  
//        }
...
- (void)emotionImageName:(CGPoint)point {
    ...
    NSString *emotionName = [itemDic allKeys][0];
    
    if (![self.selectEmotionName isEqualToString:emotionName] || self.selectEmotionName == nil) {
        
        NSString *imageName = [itemDic objectForKey:emotionName];
        UIImage *image = [self imageWithName:imageName];

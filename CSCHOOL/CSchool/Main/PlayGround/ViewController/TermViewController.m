...
- (void)viewDidLoad
{
    ...
    self.navigationItem.leftBarButtonItem = leftItem;
     ...
     self.view.backgroundColor = RGB(229, 235, 242);
    
    //初始化表格
    _tableView = [[YFJLeftSwipeDeleteTableView alloc] initWithFrame:CGRectMake(0, 0, kScreenWidth, kScreenHeight-64) style:UITableViewStylePlain];
...
- (void)_initTermView
{
    ...
    overLayView.backgroundColor = RGB(173, 176, 182);
    ...
    termView = [[UIView alloc] initWithFrame:CGRectMake(0, kScreenHeight-20-44-256, kScreenWidth, 256)];
    ...
    [cancelButton setTitleColor:Base_Color2 forState:UIControlStateNormal];
    [termView addSubview:cancelButton];
    
    UILabel *titleLabel = [[UILabel alloc] initWithFrame:CGRectMake((kScreenWidth-160)/2, 5, 160, 30)];
    titleLabel.text = @"请选择学年学期";
    titleLabel.textAlignment = NSTextAlignmentCenter;
    [termView addSubview:titleLabel];
    
    UIButton *doneButton = [[UIButton alloc] initWithFrame:CGRectMake(kScreenWidth-70, 5, 60, 30)];
    [doneButton setTitle:@"创建" forState:UIControlStateNormal];
    [doneButton setTitleColor:Base_Color3 forState:UIControlStateNormal];
    [doneButton addTarget:self action:@selector(doneAction) forControlEvents:UIControlEventTouchUpInside];
    [termView addSubview:doneButton];
    
    pickerView = [[UIPickerView alloc] initWithFrame:CGRectMake(0, 40, kScreenWidth, 216)];
...
{
    UIView *headerView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, kScreenWidth, 88)];
    UIView *view = [[UIView alloc] initWithFrame:CGRectMake(0, 0, kScreenWidth, 22)];
    view.backgroundColor = RGB(229, 235, 242);
    [headerView addSubview:view];
    
    headerView.backgroundColor = [UIColor clearColor];
    
    UIButton *semesterButton = [UIButton buttonWithType:UIButtonTypeCustom];
    semesterButton.backgroundColor = RGB(207, 213, 220);
    semesterButton.frame = CGRectMake(0, 22, kScreenWidth, 44);
...
- (void)choseTerm:(UIButton *)button
{
    ...
    XGAlertView *alertView = [[XGAlertView alloc]initWithTarget:self withTitle:@"提示" withContent:@"确定修改学期吗" WithCancelButtonTitle:@"确定" withOtherButton:@"取消"];
